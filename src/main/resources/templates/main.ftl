        ...
        <#list userMenu as items>
            <div class="panel col <#if (!items_has_next)>max_panel</#if> ">
                <a href="javascript:" data-url="${base}${items.href}">
                    <div class="panel_icon" <#if (items.bgColor != "")>style="background-color: ${items.bgColor}"
...
<div class="row">
    ...
    <div class="sysNotice col">
        ...
        <script type="text/javascript">
            ...
            $.post('${base}/admin/system/log/visitPlaceDistribution').done(function (res) {
                var mapoption = {
                    title: {
                        text: '访问文章地点分布',
                        // subtext: '纯属虚构',
            ...
            <@myindex limit = "5">
                <#if (result?size>0)>
                    <#list result as items>
                        <tr>
                            <td align="left" class="${items.content}" title="点击查看详情"
                                onclick="showcontent(this)">${items.title}</td>
                            <td>${(items.createDate?string("yyyy-MM-dd HH:mm:ss"))!}</td>
                        </tr>
                    </#list>
                </#if>
            </@myindex>
