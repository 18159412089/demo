...
<div class="layui-col-xs10">
        ...
        <div class="layui-field-box">
            <form class="layui-form" id="searchForm">
                ...
                <div class="layui-inline" style="margin-left: 15px">
                    <label>发布时间:</label>
                    <div class="layui-input-inline">
                        <input type="text" name="s_beginPublistTime" id="beginPublistTime" lay-verify="date"
                               autocomplete="off" class="layui-input">
                    </div>
                    <span>-</span>
                    <div class="layui-input-inline">
                        <input type="text" name="s_endPublistTime" id="endPublistTime" lay-verify="date"
                               autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-inline" style="margin-left: 15px">
                    <label>内容:</label>
                    <div class="layui-input-inline">
                        <input type="text" value="" name="s_content" placeholder="请输入内容"
                               class="layui-input search_input">
                    </div>
                </div>
                <br/>
                <div class="layui-inline">
                    <a class="layui-btn" lay-submit="" id="search" lay-filter="searchForm">查询</a>
                </div>
                <div class="layui-inline">
                    <button type="reset" data-type="reset" class="layui-btn layui-btn-primary">重置</button>
                </div>
                <@shiro.hasPermission name="blog:article:add">
                    <div class="layui-inline">
                        <a class="layui-btn layui-btn-normal" data-type="addBlogArticle">添加文章内容</a>
                    </div>
                </@shiro.hasPermission>
                <@shiro.hasPermission name="blog:article:delete">
                    <div class="layui-inline">
                        <a class="layui-btn layui-btn-danger" data-type="deleteBlogArticle">批量删除</a>
                    </div>
                </@shiro.hasPermission>
    ...
    <div class="layui-form users_list">
        ...
        <script type="text/html" id="barDemo">
            <@shiro.hasPermission name="blog:article:edit">
...
<script>
    ...
    function reload() {
        layui.table.reload('test', {page: {curr: $(".layui-laypage-em").next().html()}})   //这行时在当前页刷新表格的写法
    }
    ...
    layui.use(['layer', 'form', 'table', 'laydate'], function () {
        ...
        table.on('tool(demo)', function (obj) {
            ...
            if (obj.event === "del") {
                layer.confirm("你确定要删除该文章内容吗？", {btn: ['是的,我确定', '我再想想']},
                    function () {
        ...
        var t = {
            ...
            cols: [[
                ...
                {field: 'content1', title: '内容', templet: '#content'},
                ...
                {field: 'click', title: '点击量'},
                {
                    field: 'publistTime',
                    title: '发布时间',
                    templet: '<div>{{ layui.laytpl.toDateString(d.publistTime,"yyyy-MM-dd") }}</div>',
                    unresize: true
                },
        ...
        var active = {
            ...
            deleteBlogArticle:function(){
                var checkStatus = table.checkStatus('test'),
                    data = checkStatus.data;
                if(data.length > 0){
                    layer.confirm("你确定要删除这些文章吗？",{btn:['是的,我确定','我再想想']},
                        function(){
                            var d = [];
                            for(var i=0;i<data.length;i++){
                                d.push(data[i].id);
                            }
                            $.post("${base}/admin/blogArticle/delete", {"id": d}, function (res) {
                                if (res.success) {
                                    layer.msg("删除成功", {icon: 1, time: 1000}, function () {
                                        reload();
                                    });
                    ...
                    layer.msg("请选择需要删除的文章",{icon: 7},{time:1000});
                }
            },reset:function(){
                $("#searchForm")[0].reset();
                form.render();
