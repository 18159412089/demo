    ...
    <div class="layui-field-box">
        <form class="layui-form">
            <div class="layui-input-inline">
                ...
                <input type="text" value="" name="name" id="name" placeholder="请输入栏目名称"
            ...
            <@shiro.hasPermission name="blog:channel:add">
                <div class="layui-inline">
    ...
    layui.use(['tree', 'layer','form', 'table'], function () {
        ...
        var layout = [
            ...
            },
            {
                ...
                render: function (row) {
                    var text = '';
                    <@shiro.hasPermission name="blog:channel:add">text += '<a class="layui-btn layui-btn-normal layui-btn-sm" onclick="addChildMenu(' + row.id + ')"><i class="layui-icon">&#xe654;</i> 添加子栏目</a>'
                    </@shiro.hasPermission>
                    <@shiro.hasPermission name="blog:channel:edit">text += '<a class="layui-btn layui-btn-warm layui-btn-sm" onclick="editChildMenu(' + row.id + ')"><i class="layui-icon">&#xe642;</i> 编辑栏目</a>'
                    </@shiro.hasPermission>
                    <@shiro.hasPermission name="blog:channel:delete">text += '<a class="layui-btn layui-btn-danger layui-btn-sm" onclick="delMenu(' + row.id + ')"><i class="layui-icon">&#xe640;</i> 删除</a>'
                    </@shiro.hasPermission>
                    return text;
                }
            }
        ...
        form.on("submit(searchForm)", function (data) {
            $.post("${base}/admin/blogChannel/list",data.field, function (res) {
                if (res.success) {
