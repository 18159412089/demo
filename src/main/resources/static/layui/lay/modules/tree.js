;layui.define("jquery", function (e) {
    ...
    i.prototype.traverseModel = function (treeTable, parentNode, item, childrenAttrs) {
        ...
        tt = new TreeTable();
        var root = {id: 'root', children: o.options.nodes}
        o.traverseModel(tt, null, root, ['children']);
        e.addClass("layui-tree"), o.options.skin && e.addClass("layui-tree-skin-" + o.options.skin), o.treeGird(e), e.wrapInner('<tbody></tbody'), e.prepend(tableHeaderStr), e.wrapInner('<table class="layui-table"></table>'), e.wrapInner('<div class="layui-form" id="treetable"></div>'), o.on(e);
        ...
        layui.each(n, function (a, n) {
            ...
            var p = treeNode.parentId == 'root' ? null : treeNode.parentId, l = n.children && n.children.length > 0,
                str = o(['<tr class="' + (p ? "layui-hide" : "") + '" id="' + n.id + '">', function () {
                    return '<td style="width:3%">' + n.id + '</td>';
                }(), function () {
                    var ret = ""
        ...
        if (o.pid == undefined || o.pid == null) {
            e.children("input").on("change", function () {
                var childUl = e.children("ul"), checked = this.checked;
        ...
        var r = this, siblingInputs = r.siblingInputs(ele), parentId = ele.getAttribute("data-parent-id"),
            parentInput = null, bool = true, childrendInputs = null, hasOneChildrenInputCheck = false;
        ...
        var that = this;
        if (ele) {
            var parent = ele.parentElement, parents = parent.parentElement, childrens = parents.children,
                siblingInputs = [];
        } else {
            return null;
        }
        ...
        var parent = ele.parentElement, childrenInputs = [];
        ...
        var r = this, i = r.options, selector = i.elem, currentInput = e;
        if (currentInput && (currentInput.nodeName == "INPUT")) {
            var parentId = currentInput.getAttribute("data-parent-id"), parentInput = null;
            setTimeout(function () {
        ...
        var parent = ele.parentElement, that = this;
        if (selector.substr(0, 1) == "#") {
            ...
            if (parent) {
                var classnameArr = parent.className.split(" "), len = classnameArr.length, selectt = selector.substr(1),
                    hasSelector = false;
                if (len > 0) {
                    for (var i = 0; i < len; i++) {
        ...
        var that = this, randomStr = ['l', 'a', 'y', 'e', 'r', 'n', 'i'], randomNum = Math.floor(Math.random() * 6);
        return function () {
            var str = "";
        ...
        var a = this, r = (a.options, e.find(".layui-tree-spread")), nodeId = e[0].id,
            ri = e.find(".layui-tree-branch"), l = function () {
