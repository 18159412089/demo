...
public class BlogChannelController extends BaseController {
    ...
    @PostMapping("list")
    @ResponseBody
    public RestResponse list(String name,HttpServletRequest request) {
        List<BlogChannel> blogChannels = blogChannelService.selectChannelList(name);
    ...
    public RestResponse edit(BlogChannel blogChannel) {
        ...
        if (blogChannel.getId().equals(blogChannel.getParentId())) {
            return RestResponse.failure("父菜单不能是本级菜单！");
        }
        ...
        wrapper.eq("del_flag", false);
        wrapper.eq("parent_id", blogChannel.getId());
        List<BlogChannel> blogChannels = blogChannelService.selectList(wrapper);
        for (BlogChannel channel : blogChannels) {
            if (Objects.equals(channel.getId(), blogChannel.getParentId())) {
                return RestResponse.failure("父菜单不能是本级以下模块，请检查！");
            }
        }
        //重新计算parentIds
        if (blogChannel.getParentId() == null) {
            blogChannel.setLevel(1);
            blogChannel.setParentId(null);
            blogChannel.setParentIds(blogChannel.getId() + ",");
            blogChannelService.saveOrUpdateChannel(blogChannel);
        } else {
            setPidsAndLevel(blogChannel);
        }
        return RestResponse.success();
    }
   ...
   /* *
    * @Description 重新计算parentIds，level
    * @ClassName BlogChannelController
    * @param blogChannel
    * @Return void
    * @Date 2020/9/28 17:40
    * @Author huangyl
    */
    private void setPidsAndLevel(BlogChannel blogChannel) {
        BlogChannel parentChannel = blogChannelService.selectById(blogChannel.getParentId());
        if (ToolUtil.isEmpty(parentChannel)) {
            return;
        }
        blogChannel.setParentIds(parentChannel.getParentIds());
        blogChannel.setLevel(parentChannel.getLevel() + 1);
        blogChannel.setParentIds(StringUtils.isBlank(blogChannel.getParentIds()) ? blogChannel.getId() + "," : blogChannel.getParentIds() + blogChannel.getId() + ",");
        blogChannelService.saveOrUpdateChannel(blogChannel);
        setPidsAndLevel(parentChannel);
    }
