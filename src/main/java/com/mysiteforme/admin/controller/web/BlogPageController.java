...
/**
 * Created by huangyl on 2018/1/21.
 * todo:
 */
...
public class BlogPageController extends BaseController {
    ...
    public RestResponse changeClicks(@RequestParam(value = "articleId", required = false) Long articleId) {
        ...
        blogArticleService.changeClicks(articleId);
        return RestResponse.success().setData(0);
    }
    ...
    public RestResponse getOneArticleContent(Long articleId) {
        ...
        List<BlogArticle> before = beforeData.getRecords();
        resultMap.put("before", before);
        List<BlogArticle> after = afterData.getRecords();
