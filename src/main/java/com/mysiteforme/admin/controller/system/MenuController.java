...
/**
 * Created by huangyl on 2017/12/3.
 * todo:
 */
@Controller
...
public class MenuController extends BaseController {
    ...
    @ResponseBody
    public ZtreeVO menuTree() {
        List<ZtreeVO> ztreeVOs = menuService.showTreeMenus();
        ZtreeVO ztreeVO = new ZtreeVO();
        ztreeVO.setId(null);
        ztreeVO.setOpen(true);
        ztreeVO.setName("一级节点");
        ztreeVO.setChildren(ztreeVOs);
//        ztreeVOs.add(0,ztreeVO);
        LOGGER.info(JSONObject.toJSONString(ztreeVOs));
        return ztreeVO;
    }
    ...
    @ResponseBody
    public RestResponse treelist(String name, String permission) {
     ...
     * @Author huangyl
     */
    private void setPidsAndLevel(Menu menu) {
        Menu parentMenu = menuService.selectById(menu.getParentId());
        if (ToolUtil.isEmpty(parentMenu)) {
            return;
        }
        menu.setParentIds(parentMenu.getParentIds());
        menu.setLevel(parentMenu.getLevel() + 1);
