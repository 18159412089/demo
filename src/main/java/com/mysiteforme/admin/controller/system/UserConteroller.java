...
/**
 * Created by huangyl on 2017/11/21.
 */
@Controller
...
public class UserConteroller extends BaseController {
    ...
    @PostMapping("lockPassword")
    @SysLog("锁屏密码验证")
    @RequiresPermissions("sys:lockms")
    @ResponseBody
    public RestResponse lockPassword( String p) {
        ...
        //旧密码不能为空
        String pw = ToolUtil.entryptPassword(p, user.getSalt()).split(",")[0];
        if (!user.getPassword().equals(pw)) {
    ...
    @SysLog("一键清除redis缓存")
    @RequiresPermissions("sys:clearCache")
    @ResponseBody
